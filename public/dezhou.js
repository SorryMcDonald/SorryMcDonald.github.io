...
const state={user:null,room:null,rooms:[],channel:null,bannerTimer:null,acting:false,lastEventId:0,leaderboardKind:'wins'};
...
function renderRoom(){
  const room=state.room;if(!room){renderLobby();return;}$('lobbyView').hidden=true;$('tableView').hidden=false;$('createButton').hidden=true;$('joinCodeButton').hidden=true;$('leaveButton').hidden=false;
  ...
  const board=$('board');board.replaceChildren();for(let index=0;index<5;index++)board.append(cardNode(room.board[index]??null,false));renderSeats();renderActions();renderFeed();state.lastEventId=Math.max(state.lastEventId,...(room.recentEvents??[]).map((event)=>Number(event.id)),0);try{sessionStorage.setItem('texas.roomId',room.id);}catch{}
}
function renderLobby(){$('lobbyView').hidden=false;$('tableView').hidden=true;$('createButton').hidden=false;$('joinCodeButton').hidden=false;$('leaveButton').hidden=true;$('startButton').hidden=true;$('rebuyButton').hidden=true;$('roomSettingsButton').hidden=true;setText('roomTitle','公开房间');setText('statusBadge','大厅');}
...
async function refreshUser(){try{state.user=await rpc('texas_sb_me');setText('accountLabel',`${state.user.nickname} · ${money(state.user.beans)} 豆`);$('refillButton').hidden=Number(state.user.beans)!==0;}catch(error){showBanner(readableError(error));}}
...
async function loadLeaderboard(kind){try{state.leaderboardKind=kind;const entries=await rpc('texas_sb_leaderboard',{p_kind:kind});document.querySelectorAll('[data-rank]').forEach((button)=>button.classList.toggle('active',button.dataset.rank===kind));const list=$('leaderboardList');list.replaceChildren();for(const entry of entries??[]){const row=document.createElement('div');row.className='leader-row';for(const[className,text]of[['leader-rank',entry.rank],['',entry.nickname],['leader-title',entry.title],['',`${entry.wins} 胜 / ${entry.losses} 负`],['',`${money(entry.beans)} 豆`]]){const value=document.createElement('div');value.className=className;value.textContent=text;row.append(value);}list.append(row);}}catch(error){showBanner(readableError(error));}}
$('refillButton').addEventListener('click',async()=>{try{state.user=await rpc('texas_sb_refill');await refreshUser();showBanner(Number(state.user.beans)>0?'补给已到账':'牌桌仍有筹码，暂不能领取补给');}catch(error){showBanner(readableError(error));}});addEventListener('resize',()=>{if(state.room)renderSeats();});
