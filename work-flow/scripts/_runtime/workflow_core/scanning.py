...
def _load_pyproject(text):
    ...
    # Python 3.10 has no stdlib TOML parser. Extract only the evidence this
    # scanner consumes and ignore constructs that cannot be proven safely.
    data = {"project": {"scripts": {}}, "tool": {}}
    section = None
    for raw_line in text.splitlines():
        ...
        if section_match:
            ...
            if section.startswith("tool."):
                data["tool"].setdefault(section.split(".", 2)[1], {})
            continue
        ...
        if assignment:
            name = next(value for value in assignment.groups()[:3] if value is not None)
            target = next(value for value in assignment.groups()[3:] if value is not None)
            data["project"]["scripts"][name] = target
    return data
...
def _discovered_commands(root, categories):
    ...
    if pyproject.is_file():
        try:
            pyproject_data = _load_pyproject(pyproject.read_text(encoding="utf-8"))
